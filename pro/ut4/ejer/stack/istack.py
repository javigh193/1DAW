...
class IntegerStack:
    ...
    def pop(self) -> int:
        ...
        extracted_item, self.items = self[0], self[1:]
    ...
    def load_from_file(cls, path: str) -> IntegerStack:
        ...
        # f no es reversible
        with open(path) as f:
            for item in (int(line.strip()) for line in f):
                if not new_stack.push(item):
                    new_stack.expand()
                    new_stack.push(item)
        # reversed me devuelve el iterador invertido, exige __len__ y __getitem__
        new_stack.items = list(reversed(new_stack))
        return new_stack
    ...
    def __add__(self, other: IntegerStack) -> IntegerStack:
        ...
        new_stack.items = list((other)) + list((self))
        return new_stack
...
class IntegerStackIterator:
    def __init__(self, stack: IntegerStack):
        self.pointer = 0
        # ¿Debería guardar el número de items al llamar o evaluar en cada iteración?
        # self.size = len(stack)
        self.stack = stack
