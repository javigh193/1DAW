...
class IntegerQueue:
    ...
    def load_from_file(cls, path: str) -> IntegerQueue:
        ...
        por defecto"""
        new_iqueue = IntegerQueue()
        with open(path) as f:
            for line in f:
                for item in (int(item) for item in line.strip().split(",")):
                    if not new_iqueue.enqueue(item):
                        new_iqueue.expand()
    ...
    def __add__(self, other: IntegerQueue) -> IntegerQueue:
        ...
        new_iqueue = IntegerQueue(max_size=self.max_size + other.max_size)
        new_iqueue.items = list(self) + list(other)
        return new_iqueue
